...
/**
 * Build a Qoder device-flow OAuth provider definition for one edition.
 * Qoder uses a custom device flow: PKCE + nonce + machine_id are generated
 * locally, the user lands on {edition}/device/selectAccounts in the browser,
 * and we poll the edition's openapi host until a `dt-...` token appears.
 *
 * @param {"intl"|"cn"} region - intl = qoder.com (*.qoder.sh), cn = qoder.com.cn
 */
export function buildQoderProvider(region = "intl") {
  return {
    config: QODER_CONFIG,
    flowType: "device_code",
    requestDeviceCode: async (config) => {
      const { QoderService } = await import("@/lib/oauth/services/qoder");
      const flow = new QoderService(region).initiateDeviceFlow();
    ...
    pollToken: async (config, deviceCode, codeVerifier, extraData) => {
      const { QoderService } = await import("@/lib/oauth/services/qoder");
      const svc = new QoderService(region);
...
const qoder = buildQoderProvider("intl");
